...
- **headless-migrator: the open service now joins the release's own registered network, not the joining service's static default.** `OpenConfig` gains a required `network` (`MIGRATION_AGENT_NETWORK`, the release's `happ_id`), sent as `network` on `POST /v1/join`.
