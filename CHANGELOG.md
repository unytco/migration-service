...
- **Breaking.** `MIGRATION_AGENT_ALLOW_CAP_GRANT_SIGNING` / `MIGRATION_NOTARY_ALLOW_CAP_GRANT_SIGNING` permit the capability-grant path on a node with no lair to reach; they no longer override lair credentials that are present. A deployed service, whose installer renders both lair values into its EnvironmentFile, now signs through lair whichever way the variable is set.
- The lair-or-refuse decision comes from `ham`, supplied with the two variables' values, rather than being restated in each service. A refusal still names the variables, the droplet files the values come from, and the installer that renders them.
