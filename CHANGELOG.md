...
- **Breaking.** Both services connect to the conductor through lair, so connecting no longer writes to the agent's chain. Without lair signing `ham` authorizes a throwaway signing key by committing a capability grant per connect — invalid on an already-closed chain, which warrants the agent and makes its signed close unfetchable. A service that cannot get lair credentials now exits at startup instead. An existing deployment fails until its environment sets `MIGRATION_AGENT_LAIR_URL` / `MIGRATION_AGENT_LAIR_PASSPHRASE` (`MIGRATION_NOTARY_*` for the daemon), or deliberately opts back in to the old path with `MIGRATION_AGENT_ALLOW_CAP_GRANT_SIGNING=1` / `MIGRATION_NOTARY_ALLOW_CAP_GRANT_SIGNING=1`.
