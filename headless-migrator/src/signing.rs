//! How this service's zome calls are signed, decided from the environment
...
//! connect too many permanently destroys the agent's ability to migrate. Lair is
//! therefore the only default, a service that cannot get it refuses to start,
//! and the chain-writing path is reachable only by setting
//! [`ALLOW_CAP_GRANT_VAR`].
...
/// The signer every `ham` connection this service makes is built with.
#[derive(Clone)]
pub enum Signing {
    ...
    /// the agent's chain per connect. Reachable only through the opt-in.
    CapGrant,
}
...
impl Signing {
    ...
    ///
    /// The opt-in wins over present lair credentials: a deployed service always
            ...
            Self::CapGrant => {
                tracing::warn!(
                    ...
                    "signing WITHOUT lair: this connect commits a capability grant to the \
                     agent's chain, which is invalid on a closed chain"
                );
...
/// The error a service without lair signing dies with. It carries everything an
...
fn refusal(url_present: bool, passphrase_present: bool) -> anyhow::Error {
    anyhow::anyhow!(
        "refusing to connect: lair signing is required and unavailable ({LAIR_URL_VAR} is {}, \
         ...
         grant to this agent's chain. On a chain that is already closed that action is invalid, \
         ...
         installers (setup-migrate-close.sh / setup-migrate-open.sh) read them off the node and \
         render them into this service's EnvironmentFile.\n\
...
mod tests {
    ...
    fn half_the_lair_credentials_refuses_too() {
        for (url, passphrase) in [
            ...
            // The message distinguishes which half is missing, so the operator
            // is not sent looking at both.
