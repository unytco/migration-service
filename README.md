...
- **`notary-daemon/`** — a Rust `axum` + [`ham`](https://github.com/unytco/ham) service. Runs co-located with a Holochain conductor that keeps serving the old (from-DNA) network. Its `/v1/fetch-close` calls the alliance `read_predecessor_close` zome fn — a pure read of the agent's committed close; the M-of-N notary signatures *inside* the package carry the trust, collected on-chain before the close. The daemon signs its own zome calls through the node's lair, so connecting commits nothing to its chain, and it can sign for **nobody else**: there is no notarization path in it. Exposed to the router via a Cloudflare Tunnel; healthy only when both the conductor and its app cell answer.
