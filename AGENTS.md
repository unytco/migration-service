...
cd migration-agent && cargo build --release
...
nix develop -c bash -c 'cd migration-agent && cargo build --release --target x86_64-unknown-linux-musl'
```
...
cd migration-agent && cargo fmt && cargo fmt --check
