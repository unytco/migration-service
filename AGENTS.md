...
- **Branching:** integrate on `develop`; release by merging `develop → main` (the merge triggers the router deploy). `develop-0.7` is the parallel **Holochain 0.7 integration branch** — temporary, mirroring the workshop umbrella's `main-0.7` carve-out, and it goes away when the 0.7 stack merges down. CI (`ci.yml`) runs on push/PR to all three; only `main` deploys the router (`deploy.yml`).
